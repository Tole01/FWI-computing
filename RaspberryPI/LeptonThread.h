...
signals:
    ...
    void updateTable(QStringList);
    void updateTemperatures(const std::vector<std::vector<float>>& temps); // Cambiado a float

private:
    void log_message(uint16_t, std::string);
    void sendToESP32(const std::vector<std::vector<float>>& data); // Cambiado a float
